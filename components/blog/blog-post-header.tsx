...
export function BlogPostHeader({ post }: BlogPostHeaderProps) {
  ...
  return (
    <header ref={headerRef} className="space-y-6">
      ...
      {post.category && (
        <div ref={categoryRef}>
          <Link href={`/blog/category/${post.category.slug}`}>
            <Badge
              variant="secondary"
              className="text-sm px-3 py-1 hover:bg-primary/10 transition-colors cursor-pointer"
              style={{
      ...
      {/* Meta Information */}
      <div ref={metaRef} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 text-sm text-muted-foreground">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs truncate">{post.author_bio}</div>
            </div>
          </div>
      ...
      {/* Featured Image */}
      {post.featured_image && (
        <div ref={imageRef} className="relative aspect-[16/9] overflow-hidden rounded-lg">
          <Image
